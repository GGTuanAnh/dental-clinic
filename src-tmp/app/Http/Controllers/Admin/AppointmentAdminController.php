...
class AppointmentAdminController extends BaseAdminController
{
	...
	public function index(Request $request)
	{
		...
		if ($period = $request->string('period')->toString()) {
			...
			switch ($period) {
				case 'today':
					$query->whereDate('appointment_at', $now->format('Y-m-d'));
					break;
				case '7days':
					$query->whereDate('appointment_at', '>=', $now->copy()->subDays(7));
					break;
				case '1month':
					$query->whereDate('appointment_at', '>=', $now->copy()->subMonth());
					break;
				case '3months':
					$query->whereDate('appointment_at', '>=', $now->copy()->subMonths(3));
					break;
				case '6months':
					$query->whereDate('appointment_at', '>=', $now->copy()->subMonths(6));
					break;
				case '1year':
					$query->whereDate('appointment_at', '>=', $now->copy()->subYear());
					break;
			}
		} else {
			// Bộ lọc tùy chỉnh from-to (nếu không dùng period)
